...
void acp_dump(FILE *fd, int type, int protocol, uint32_t src_ip, uint16_t src_port, uint32_t dst_ip, uint16_t dst_port, char *data, int len, uint32_t *seq1, uint32_t *ack1, uint32_t *seq2, uint32_t *ack2) {
    ...
    static const char ethdata[15] = "\x00\x00\x00\x00\x00\x00"  /* dest   */
    ...
    if(!ack1) ack1 = &lame_tmp[1];
    if(!seq2) seq2 = &lame_tmp[2];
    if(!ack2) ack2 = &lame_tmp[3];
    if(type     < 0) type     = 1;  // WSASocket!
    if(protocol < 0) protocol = 6;  // WSASocket!
    if(type    == 4) type     = 2;  // SOCK_RDM
    if(type    == 5) type     = 1;  // SOCK_SEQPACKET
    if(!type && !protocol) {
        protocol = 6;
