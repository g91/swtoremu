...
bool swGetKeys::Getloginfiles(char* LoginFile)
{
	HANDLE hKeysFile = CreateFile(LoginFile, 
	...
	while(!GetSWTORKeys(hKeysFile, 0x00));

	CloseHandle(hKeysFile);
	return true;
}

bool swGetKeys::Getgamefiles(char* ServerFile)
{
	HANDLE hKeysFileGame = CreateFile(ServerFile, 
		GENERIC_READ | GENERIC_WRITE,
		FILE_SHARE_READ,
		0,
		CREATE_ALWAYS,
	...
	while(!GetSWTORKeys(hKeysFileGame, 0x04));
