...
namespace Ookii.Dialogs.Wpf
{
    ...
    public partial class TaskDialog : Component, IWin32Window
    {
        ...
        {
            if( !IsDialogRunning )
                throw new InvalidOperationException(NexusLauncher.Properties.Resources.TaskDialogNotRunningError);
        ...
        private TaskDialogButton ShowDialog(IntPtr owner)
        {
            if( !OSSupportsTaskDialogs )
                throw new NotSupportedException(NexusLauncher.Properties.Resources.TaskDialogsNotSupportedError);

            if( IsDialogRunning )
                throw new InvalidOperationException(NexusLauncher.Properties.Resources.TaskDialogRunningError);

            if( _buttons.Count == 0 )
                throw new InvalidOperationException(NexusLauncher.Properties.Resources.TaskDialogNoButtonsError);
        ...
        private List<NativeMethods.TASKDIALOG_BUTTON> SetupButtons()
        {
            ...
            {
                if( button.Id < 1 )
                    throw new InvalidOperationException(NexusLauncher.Properties.Resources.InvalidTaskDialogItemIdError);
                ...
                if( button.ButtonType == ButtonType.Custom )
                {
                    if( string.IsNullOrEmpty(button.Text) )
                        throw new InvalidOperationException(NexusLauncher.Properties.Resources.TaskDialogEmptyButtonLabelError);
        ...
        private void CheckCrossThreadCall()
        {
            ...
            if( handle != IntPtr.Zero )
            {
                ...
                if( windowThreadId != threadId )
                    throw new InvalidOperationException(NexusLauncher.Properties.Resources.TaskDialogIllegalCrossThreadCallError);
            }
        }
