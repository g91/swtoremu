            ...
            set
            {
                if( base.Id != value )
                {
                    if( _type != ButtonType.Custom )
                        throw new InvalidOperationException(SCPTExtractor.Properties.Resources.NonCustomTaskDialogButtonIdError);
                    base.Id = value;
                }
            }
        }
            ...
            if( type != ButtonType.Custom && Owner != null )
            {
                foreach( TaskDialogButton button in Owner.Buttons )
                {
                    if( button != this && button != itemToExclude && button.ButtonType == type )
                        throw new InvalidOperationException(SCPTExtractor.Properties.Resources.DuplicateButtonTypeError);
                }
            }
        }
    }
