...
namespace Ookii.Dialogs.Wpf
{
    ...
    public abstract partial class TaskDialogItem : Component
    {
        ...
        {
            if( Owner == null )
                throw new InvalidOperationException(SCPTExtractor.Properties.Resources.NoAssociatedTaskDialogError);
                ...
                if( items != null )
                {
                    foreach( TaskDialogItem item in items )
                    {
                        if( item != this && item != itemToExclude && item.Id == id )
                            throw new InvalidOperationException(SCPTExtractor.Properties.Resources.DuplicateItemIdError);
                    }
                }
            }
        }
