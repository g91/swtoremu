...
namespace swtorMeters
{
    ...
    class RequestMultipleRPC : public BasicSerializable <uint32_t, uint32_t, BasicVector <uint32_t, BasicVector <uint32_t, uint8_t>>>
    {
        public:
            ACCESSOR_1(0, uint32_t, MessageID)
            ACCESSOR_1(1, uint32_t, StreamID)
            ACCESSOR_1(2, std::vector <std::vector <uint8_t>>, Requests)
    };
};
